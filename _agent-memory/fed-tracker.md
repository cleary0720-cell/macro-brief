...
- EFFR RATE TRANSITION NOTE: When the Fed hikes at 2pm ET, the implementation note says "effective [next business day]". So:
  ...
  - Day after hike EFFR (e.g. Sep 17) = first full day at new rate (~3.88% with IORB 3.90%)
  - This means on day-after-hike runs, FRED/EFFR searches showing hike-day's rate = 3.63% (old regime) is CORRECT and EXPECTED
...
### September 18, 2026 — THURSDAY (2 days after FOMC decision)
- Target range: 3.75% – 4.00% (no change; hiked Sep 16)
- Effective rate: ~3.88% (Sep 17 EFFR published Sep 18 ~9am ET; IORB 3.90% since Sep 17; estimated)
  - Sep 16 EFFR (published Sep 17): CONFIRMED 3.63% (hike effective Sep 17 per implementation note; Sep 16 transactions were pre-hike)
  - Sep 17 EFFR (published Sep 18): ~3.88% (first full day at new range; IORB 3.90%; exact figure not confirmed in search but logically derived)
- Next meeting: October 27–28, 2026 (decision October 28 at 2pm ET = 18:00 UTC)
- CME FedWatch October 28 hike: ~55% / hold ~45% (55.1% from search; unchanged from Sep 17)
- CME FedWatch December 9 second 2026 hike: ~64% (unchanged from Sep 17)
- New FOMC row added: NO (no new meeting)
- MEANS-FOR-YOU: not updated (rate unchanged today)
- JS countdown: 2026-10-28T18:00:00Z (unchanged; correct)
- Changes made:
  - "Last updated" → September 18, 2026
  - Card 1 note: "Sep 17, 2026 (today)" → "Sep 17, 2026" with Sep 16 EFFR 3.63% confirmed; added "Sep 18, 2026 (today)" with Sep 17 EFFR ~3.88%, CME stable
  - Card 2 note: Same corrections — Sep 17 entry corrected to 3.63% EFFR; Sep 18 (today) entry added
- Sources: WebSearch FRED/NY Fed EFFR snippets (3.63% Sep 16; ~3.88% Sep 17 est.); CME FedWatch 55.1% Oct 28 via WebSearch snippet; federalreserve.gov implementation note confirmed IORB 3.90% effective Sep 17

## CRITICAL NOTE for NEXT RUN (Sep 19 Fri morning):
- **EFFR for Sep 17**: ~3.88% (estimated; first full day under new regime). Confirm exact figure when published Sep 18 ~9am ET — should appear in FRED/NY Fed by next run.
  - If Sep 19 morning run: EFFR for Sep 18 published ~9am ET; also expected ~3.88%
  - Update EFFECTIVE RATE display from "~3.88%" to confirmed figure once we can confirm
...
- Market probabilities entering next run:
  - October 28 hike: ~55% / hold ~45% (CME FedWatch, Sep 18)
  - December 9 second hike: ~64% (CME FedWatch, Sep 18)
